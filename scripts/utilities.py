...
def convert_mp4_to_jpg(
    dir_path: str, txt_to_remove: str = ".mp4", txt_to_insert: str = ""
):
    ...
    for filename in os.listdir(dir_path):
        ...
        try:
            # Temporary fix for duplicate .mp4 in file names generated by Adobe
...
def count_files_in_dir(path, extensions=None):
    """Counts the number of files in a directory and subdirectories that match the given extensions.

    Args:
        path (str): Path to the directory to search for files.
        extensions (list of str, optional): List of file extensions to count. If None, all files will be counted.

    Returns:
        int: Number of files in the directory and subdirectories that match the given extensions.
    """
    count = 0
...
def get_random_sample(read_path: str, write_path: str, sub_sample: float = 0.05):
    ...
    files = [
        f for f in os.listdir(read_path) if os.path.isfile(os.path.join(read_path, f))
    ]

    # select sub-sample of the files randomly
    random_files = np.random.choice(files, int(len(files) * sub_sample))
    files = [
        shutil.copy(os.path.join(read_path, file), write_path) for file in random_files
    ]
